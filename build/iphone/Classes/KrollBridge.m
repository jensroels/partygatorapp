...
NSString * facebook$ModuleRequireFormat = @"(function(exports){"
...
{
	// create facebook global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "facebook" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_facebook = [[facebookObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_facebook];
	
	NSString *_facebookNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _facebookNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_facebook valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:facebook$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	@throw [NSException exceptionWithName:@"org.facebook.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
